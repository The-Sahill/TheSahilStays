...
export default function FinancialDesk() {
  const [filter, setFilter] = useState('all'); // 'all' أو 'month'
  
  // بيانات لكل السجلات (الإجمالي العام)
  ...
  // 1. جلب البيانات المالية الإجمالية لكل الأوقات
  useEffect(() => {
    const getFinancialData = async () => {
      try {
        const { data } = await axios.get(`${apiUrl}/financialData`);
        if (data.success) {
          setFinancialData({
  ...
  // 2. جلب البيانات المالية الخاصة بالشهر الحالي
  useEffect(() => {
    const getFinancialDataCurrentMonth = async () => {
      try {
        const { data } = await axios.get(`${apiUrl}/getFinancialDataCurrentMonth`);
        if (data.success) {
    ...
    getFinancialDataCurrentMonth();
  }, []);

  // 3. جلب بيانات المركبات
  useEffect(() => {
    const financialDataForTypes = async () => {
      try {
        const { data } = await axios.get(`${apiUrl}/financialDataForTypes`);
        if (data.success) {
  ...
  // تحديد أي بيانات سيتم عرضها بناءً على الفلتر المختار (كل الوقت أو الشهر الحالي)
  const currentDisplayData = filter === 'month' ? financialDataCurrentMonth : financialData;
  ...
  return (
    <div className="min-h-screen w-full bg-[#fbfaf6] text-[#1b2a32] font-sans">
      ...
      <main className="max-w-[1600px] mx-auto p-6 md:p-10 flex flex-col gap-8">
        ...
        {/* العنوان والوصف مع زر التبديل بين الكل والشهر الحالي */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          {/* زر التبديل لتصفية البيانات حسب الشهر أو الكل */}
          <div className="flex items-center gap-2 bg-white border border-gray-200 p-1 rounded-xl shadow-sm">
            <button 
              onClick={() => setFilter('all')}
              className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${filter === 'all' ? 'bg-[#1b2a32] text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              إجمالي الكل
            </button>
            <button 
              onClick={() => setFilter('month')}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${filter === 'month' ? 'bg-[#1b2a32] text-white' : 'text-gray-600 hover:bg-gray-50'}`}
            >
              <Calendar className="w-4 h-4" /> هذا الشهر
            </button>
          </div>
        </div>

        {/* بطاقات الإحصائيات العليا (تتغير تلقائياً حسب الفلتر المختار) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-start">
            <div>
              ...
              <p className="text-4xl font-semibold text-[#1b2a32] mb-2">{currentDisplayData.totalCost}</p>
          ...
          <div className="bg-[#e0eae5] p-6 rounded-3xl border border-emerald-200/50 shadow-sm flex justify-between items-start">
            <div>
              ...
              <p className="text-4xl font-semibold text-emerald-950 mb-2">{currentDisplayData.profit}</p>
        ...
        {/* قسم الرسم البياني وطابور التسوية */}
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="xl:col-span-5 bg-[#1b2a32] text-white p-8 rounded-3xl flex flex-col justify-between">
            <div>
              ...
              <p className="text-5xl font-semibold text-white mb-2">التسويات المالية</p>
              <p className="text-yellow-400 font-medium mb-8">مراجعة ومتابعة العمليات المالية المعلقة</p>
              <div className="bg-white/5 p-5 rounded-2xl border border-white/10 mb-6">
                <p className="text-xs text-gray-300 leading-relaxed">
                  يمكنك مراجعة العمليات المالية وتحديث حالة التسوية لضمان دقة السجلات والمعاملات.
                </p>
              </div>
            </div>
          </div>
